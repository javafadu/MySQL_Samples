...
insert into students values (100,'Ali Can',15,'2007-02-02');
...
-- Tam veri girişi
INSERT INTO students VALUES(101, 'ALI CAN', 13, '2021-08-11');
...
update students set student_age=21 where student_id=101; -- hata verir yas 0-20 arasinda olmasi gerekiyor tanimlamistik
update students set student_name='ali can' where student_id=101; -- hatali oldu bunu kabul etmemesi gerekiyordu
...
-- hata verir icerdekiler unique degil
...
select name, sum(salary) as total_salary from workers group by name order by total_salary DESC;


-- Çalışanların eyaletlere gör çalışan sayılarını büyükten küçüğe göre listeleyiniz.
select state, count(*) as calisan_sayisi from workers group by state order by calisan_sayisi DESC;


-- Şirketlerde 2000$ fazla oalan çalışan sayılarını listeleyiniz.  
select company, count(*) as calisan_sayisi_maas2000denbuyuk from workers where salary>2000 group by company;


-- Her şirket için minimum ve maximum maasları listeleyiniz.
select company, min(salary) as min_salary, max(salary) as max_salary from workers group by company;


-- HAVING CLAUSE
-- Find the total salary if it is greater than 2500 for every employee
select name, sum(salary) as total_salary from workers group by name having total_salary>2500;
...
select state, count(*) as calisan_sayisi from workers group by state having calisan_sayisi>1;




-- Şirketlerde 2000$ dan fazla olan minumum maaşları listeleyiniz.
select company, min(salary) as min_maas from workers group by company having min_maas>2000;



-- Her eyalette 3000$ altındaki maksimum maaşları listeleyiniz.
select state, max(salary) as max_maas from workers group by state having max_maas<3000;
