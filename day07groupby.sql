...
select ulke, count(id) from personel group by ulke;
...
-- SORU05: Personelin, ulkelere göre ve şehirler göre gruplayarak sorgulayın
select ulke,sehir,count(id) as kisi_sayisi from personel group by ulke,sehir order by ulke,sehir;


-- SORU06: Personelin, ulkelere göre ve şehirler göre calışan sayısını sorgulayın.
select ulke,sehir,count(id) as calisan_sayisi from personel group by ulke,sehir order by ulke,sehir;


-- SORU07: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select ulke, cinsiyet, count(id) as calisan_sayisi, round(avg(yas),2) as ortalama_yas from personel group by ulke,cinsiyet;


-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.
select ulke, cinsiyet, count(id) as calisan_sayisi, round(avg(yas),2) as ortalama_yas from personel where maas>30000 group by ulke,cinsiyet order by ulke,cinsiyet;
...
select ulke, cinsiyet, count(id) as calisan_sayisi, round(avg(yas),2) as ortalama_yas from personel where maas>30000 group by ulke,cinsiyet order by ortalama_yas DESC;
...
select isim,urun_adi,sum(urun_miktari) as toplam_urun_miktari  
from manav 
group by isim,urun_adi 
having toplam_urun_miktari>=3 
order by toplam_urun_miktari DESC;


-- aggregate fonksiyonlar ile ilgili bir koşul koymak için group by dan sonra having kullanılır.
...
/*==================================== DISTINCT ======================================
	...
	SYTAX
    -----
    SELECT DISTINCT sütun_adı1, sürun_adı2 ...
    FROM tablo_adı;
======================================================================================= */
