...
select * from manav;
select isim, sum(urun_miktari) as toplam_miktar from manav group by isim;
...
select urun_adi, min(urun_miktari) as min_urun_miktari, max(urun_miktari) as max_urun_miktari from manav group by urun_adi order by max_urun_miktari DESC;


-- SORU4: kisi ismine ve urun adına göre satılan ürünlerin toplamını 
